...
% iv_fis = 'sp500_hf';   % 2001.9.1 missing
iv_mon = 'ff4_hf'; 
...
for i_p=1:np % For each lag length...
    ...
    the_shock = the_chol(:,2); % suppose the shock on the 3^{th} variable
    % the_By_full: the coefficient matrix of different lags
    % u1 = a11*e1, u2 = a21*e1 + a21*e2.... (only e1 is shock)
    the_irf_var = IRF_SVAR(the_By_full,the_shock,maxhorz);
...
xlabel('Horizon (monthly)','interpreter','latex','FontSize',20)
ylabel('\% deviation','interpreter','latex','FontSize',20)
legend({'SVAR-ratio','LP-IV'},'Location','Northeast','fontsize',20,'interpreter','latex')
xlim([0 maxhorz])
ylim(plot_ylim)
grid on
hold off
